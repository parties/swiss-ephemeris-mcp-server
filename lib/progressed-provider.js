...
// Ascendant and moving house cusps need actual swetest -house lookups (obliquity, ARMC,
// the fictitious-longitude trick), which lib/house-frame.js performs; the providers over
// it live in index.js next to the rest of findEvents' provider wiring.

import { seriesFor as ephemerisSeriesFor, positionAt as ephemerisPositionAt, samplesFrom as ephemerisSamplesFrom } from './ephemeris-series.js';
...
export function progressedBodyProvider(body, { birthJd, yearLengthDays }) {
  return {
    // What this provider samples underneath, for a caller composing two real-body providers
    // that wants both bodies out of one swetest spawn (SUP-387 - see index.js's
    // pairPrefetchFor). Only a plain real body carries this; the progressed MC/Ascendant
    // and moving-cusp providers below and in index.js deliberately do not, which is how a
    // caller tells that there is no single swetest call covering both sides.
    batchSource: {
      body,
      ephemerisJdFor: (targetJd) => ephemerisJdForTarget(targetJd, birthJd, yearLengthDays),
      scaleSpeed: (speed) => speed / yearLengthDays,
    },
    ...
    // The seam's batched read (SUP-390 - see lib/event-search.js's header). The target ->
    // ephemeris map is affine, so an evenly spaced grid in target time is an evenly spaced
    // grid in ephemeris time and the whole batch is still one swetest spawn. The JDs handed
    // back are rebuilt in TARGET space (`startJd + i*stepDays`) rather than mapped back from
    // the ephemeris grid: the caller's bracket arithmetic lives in target time, and a JD
    // that took the round trip would differ from it in the last bit for no reason.
    samplesFrom(startJd, stepDays, count) {
      const ephStart = ephemerisJdForTarget(startJd, birthJd, yearLengthDays);
      const ephStep = stepDays / yearLengthDays;
      return ephemerisSamplesFrom(body, ephStart, ephStep, count).map((row, i) => ({
