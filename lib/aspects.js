...
// instead — see matchAspectsForPair.
...
function matchAspectsForPair(a, b, aspectDefs, orbsByClass, orbModel = 'moiety', moieties = MOIETIES, aspectMultipliers = ASPECT_MULTIPLIERS) {
...
export function orbAllowedFor(settings, nameA, nameB, aspectName) {
  const { orbModel, orbsByClass, moieties, aspectMultipliers } = settings;
  return orbModel === 'moiety'
    ? (moieties[nameA] + moieties[nameB]) * aspectMultipliers[aspectName]
    : Math.min(orbsByClass[orbClassForBody(nameA)][aspectName], orbsByClass[orbClassForBody(nameB)][aspectName]);
}
...
export function calculateNatalAspects(bodiesWithLonSpeed, options = {}) {
  ...
  for (let i = 0; i < bodiesWithLonSpeed.length; i++) {
    for (let j = i + 1; j < bodiesWithLonSpeed.length; j++) {
      aspects.push(...matchAspectsForPair(bodiesWithLonSpeed[i], bodiesWithLonSpeed[j], aspectDefs, orbsByClass, orbModel, moieties, aspectMultipliers));
    }
  }
