...
function matchAspectsForPair(a, b, aspectDefs, orbsByClass, orbModel = 'class') {
  ...
  for (const [aspectName, targetAngle] of Object.entries(aspectDefs)) {
    ...
    const orbAllowed = orbModel === 'moiety'
      ? (MOIETIES[a.name] + MOIETIES[b.name]) * ASPECT_MULTIPLIERS[aspectName]
...
function resolveAspectSettings(options = {}) {
  ...
  const aspectDefs = includeMinor
    ...
    : { ...MAJOR_ASPECTS };
  return { orbsByClass, aspectDefs, orbModel };
}
...
export function calculateNatalAspects(bodiesWithLonSpeed, options = {}) {
  ...
  const { orbsByClass, aspectDefs, orbModel } = resolveAspectSettings(options);
  ...
  for (let i = 0; i < bodies.length; i++) {
    for (let j = i + 1; j < bodies.length; j++) {
      aspects.push(...matchAspectsForPair(bodies[i], bodies[j], aspectDefs, orbsByClass, orbModel));
    }
  }
...
export function calculateCrossChartAspects(bodiesA, bodiesB, options = {}) {
  ...
  for (const a of bodiesA) {
    for (const b of bodiesB) {
      aspects.push(...matchAspectsForPair(a, b, aspectDefs, orbsByClass, orbModel));
    }
  }
