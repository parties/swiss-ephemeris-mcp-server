...
// orb_model selects how a pair's allowed orb is derived. 'class' (default) is the fixed
// per-class table above (ORB_CLASSES). 'moiety' sums each body's individually resolved
// half-orb (MOIETIES) and scales by the aspect's multiplier (ASPECT_MULTIPLIERS) instead —
// see matchAspectsForPair.
...
function matchAspectsForPair(a, b, aspectDefs, orbsByClass, orbModel = 'class', moieties = MOIETIES, aspectMultipliers = ASPECT_MULTIPLIERS) {
  const { signedDiff, separation } = normalizeSeparation(a.longitude, b.longitude);
  const orbsA = orbModel === 'moiety' ? null : orbsByClass[orbClassForBody(a.name)];
  const orbsB = orbModel === 'moiety' ? null : orbsByClass[orbClassForBody(b.name)];
  const matches = [];
...
export function invalidOrbOverrideKeys(orbOverrides, orbModel = 'class') {
...
function resolveAspectSettings(options = {}) {
  ...
  const orbsByClass = orbModel === 'moiety'
    ? null
    : {
  ...
  const aspectDefs = includeMinor
    ...
    : { ...MAJOR_ASPECTS };
  return { orbsByClass, aspectDefs, orbModel, moieties, aspectMultipliers };
}
...
export function calculateNatalAspects(bodiesWithLonSpeed, options = {}) {
  ...
  const { orbsByClass, aspectDefs, orbModel, moieties, aspectMultipliers } = resolveAspectSettings(options);
  ...
  for (let i = 0; i < bodies.length; i++) {
    for (let j = i + 1; j < bodies.length; j++) {
      aspects.push(...matchAspectsForPair(bodies[i], bodies[j], aspectDefs, orbsByClass, orbModel, moieties, aspectMultipliers));
    }
  }
...
export function calculateCrossChartAspects(bodiesA, bodiesB, options = {}) {
  ...
  for (const a of bodiesA) {
    for (const b of bodiesB) {
      aspects.push(...matchAspectsForPair(a, b, aspectDefs, orbsByClass, orbModel, moieties, aspectMultipliers));
    }
  }
