...
// One aspect contact (spec §3.2): every exact pass of `provider` against
// `natalLongitude + aspectAngle` within [startJd, endJd], the orb envelope
// (enters_orb/leaves_orb, `*_truncated` when clipped by the window rather than found),
// and `closest_approach`. `segments`/`stations` come from scanTransitingBody for this
// same provider/window - callers reuse one scan across every (target, aspect) pair for a
// given transiting body rather than rescanning per pair.
//
// Returns null when the body never comes within orb during the window at all (as
// opposed to an empty `passes` array, which is a real result - see spec Q4: a station
// that gets close without perfecting still has an orb envelope, just no exact pass).
export function findContact({ provider, segments, stations, natalLongitude, aspectAngle, orbAllowed, startJd, endJd }) {
  ...
  const passes = exactCrossings.map((c) => ({
  ...
  const entersOrbTruncated = gStart <= orbAllowed;
  const leavesOrbTruncated = gEnd <= orbAllowed;

  let orbCrossings = [];
  if (!entersOrbTruncated || !leavesOrbTruncated) {
    orbCrossings = [
  ...
  if (!entersOrbTruncated && !leavesOrbTruncated && orbCrossings.length === 0) {
    return null; // never comes within orb during the window
  }

  const entersOrbJd = entersOrbTruncated ? startJd : orbCrossings[0].jd;
  const leavesOrbJd = leavesOrbTruncated ? endJd : orbCrossings[orbCrossings.length - 1].jd;

  // closest_approach is computed, not searched (spec Q4): on a monotone segment |g| is
  // minimised at a segment endpoint or a root, so the candidate set is exactly the exact
  // passes (orb 0), the stations inside the period, and the window boundaries.
  const candidates = [
    ...passes.map((p) => ({ jd: p.jd, orb: 0, stationary: false })),
    ...stations
      .filter((s) => s.jd >= startJd && s.jd <= endJd)
  ...
  return {
    ...
    enters_orb: isoFromJd(entersOrbJd),
    leaves_orb: isoFromJd(leavesOrbJd),
...
export function natalContactsFor(longitude, targets, aspectAngles, orbAllowedFor) {
  ...
  for (const target of targets) {
    for (const [aspectName, angle] of Object.entries(aspectAngles)) {
      const orb = Math.abs(wrap180(longitude - target.longitude - angle));
