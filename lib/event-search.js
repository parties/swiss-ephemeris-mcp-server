...
// endJd, stepDays), positionAt(jd) }` plus the optional `samplesFrom(startJd, stepDays,
// count)` below, already bound to one body - and, where an orb matters, a plain
// `orbAllowed` number already resolved by the caller (see
...
//
// `samplesFrom(startJd, stepDays, count)` (SUP-390) is the seam's optional BATCHED read:
// `count` samples on the arithmetic grid `startJd + i*stepDays`, at a cost much closer to
// one sample than to `count` of them - a provider backed by swetest gets them all from one
// `-jX -sSTEP -nN` spawn. A provider that has no such form (the progressed Ascendant and
// moving house cusps, where a single sample is a `-house` computation rather than a row of
// a `-p` scan) simply omits it, and every consumer here falls back to `positionAt`. So it
// is a cost hint, exactly like `prime`/`isPrimed` below: never a source of a result that
// `positionAt` would not have produced.
...
// Bisection steps a single batched spawn buys, when the provider has `samplesFrom` (SUP-390).
// A batch of 2^k - 1 samples advances the bracket exactly k halvings, so this is a pure
// cost knob - it changes how many spawns the refinement takes, never where it lands.
//
// Chosen from the measured shape of a swetest spawn on this machine: ~2.1 ms of fixed cost
// (fork/exec/ephemeris-file open) against ~15 us per additional row, flat out to thousands
// of rows. At k = 6 a batch is 63 rows - about 1.5 spawns' worth of wall clock doing 6
// spawns' worth of work. Going wider keeps helping only until the rows outweigh the fork:
// k = 5/6/7 all land within ~5% of each other for a day-wide bracket (12.8/12.2/12.0 ms
// against bisection's 44 ms), and k = 11 - two spawns of 2047 rows - is back up at 65 ms.
// Six sits in the flat middle of that curve at both rates: 4 spawns for the transit rate's
// day-wide bracket, 5 for the progressed rate's tropical-year one.
const STATION_BATCH_BITS = 6;

// Refine a station bracket (speed sign change between jdLo and jdHi, <=1 day apart at the
// transit rate, one tropical year of target time at the progressed one) down to
// JD_TOLERANCE.
//
...
//
// SUP-390 left the algorithm alone and changed only where its samples come from. Bisection
// probes a DYADIC grid: k halvings of [lo, hi] can only ever visit points of the form
// `lo + j*(hi-lo)/2^k`, and which of those 2^k sub-intervals it ends in is decided entirely
// by the sign of the speed there. So all 2^k - 1 candidates are fetched in ONE spawn and
// the k steps are then REPLAYED against them in memory - one spawn instead of k, with no
// interpolation, no new tolerance, and no change to the monotone-segment argument at the
// top of this file (this refines INSIDE a bracket the coarse scan already found; it does
// not change how brackets are found).
//
// Replayed, emphatically, and not scanned. It is tempting to take the leftmost sign change
// in the batch and skip the index arithmetic, and that is wrong for a reason worth
// recording: `sign(speed) === sign(speedLo)` is NOT monotone across the bracket. Sampling
// the printed speed at 0.25s resolution through Pluto's 2027-05-08 station gives
//
//     ...12:54:04.00 -> 0.0000000   12:54:04.25 -> 0.0000001   12:54:05.75 -> 0.0000000
//        12:54:06.25 -> 0.0000001   12:54:06.50 -> 0.0000000   12:54:07.00 -> 0.0000001...
//
// - the plateau does not have one clean edge, it DITHERS across a few seconds as the true
// speed grazes the 7th decimal. Bisection lands on whichever flip its own path finds; a
// left-to-right scan lands on the first one, up to 2.7 seconds earlier. Measured over 52
// transit-rate stations, replaying gives a station JD bit-identical to the scalar loop's
// while scanning moved eight of them - three by a whole reported second. At the progressed
// rate, where the coarse step is a tropical year and the grid arithmetic is therefore not
// exactly dyadic, replaying still agrees to within 8e-5 s (about two ulps of a JD double),
// against a JD_TOLERANCE of 0.05 s and a reported resolution of 1 s.
//
// Two properties of the batched read make the replay exact rather than approximate:
// swetest steps its grid as `t0 + i*step` to full double precision, and - measured across
// the Moon, Mercury and Neptune at steps from 1/64 day down to 1e-8 day - the speed it
// prints for a grid row is bit-identical to the speed it prints for a standalone query at
// the same instant. Speed is the only thing this function reads. (Longitude differs by up
// to 2 units in the 7th decimal at the finest steps, which is why the station's REPORTED
// longitude still comes from a scalar positionAt in scanTransitingBody, never from a grid
// row.)
//
// The scalar loop below is the fallback for a provider with no `samplesFrom`, and it is
// unchanged - the progressed Ascendant and moving cusps take that path, since one sample
// there is a `-house` chart computation with no batched form to exploit.
function refineStationJd(provider, jdLo, jdHi, speedLo) {
  const signLo = Math.sign(speedLo);
  let lo = jdLo;
  let hi = jdHi;

  if (typeof provider.samplesFrom === 'function') {
    while (hi - lo > JD_TOLERANCE) {
      const bits = Math.min(STATION_BATCH_BITS, Math.max(1, Math.ceil(Math.log2((hi - lo) / JD_TOLERANCE))));
      const divisions = 2 ** bits;
      const step = (hi - lo) / divisions;
      const rows = provider.samplesFrom(lo + step, step, divisions - 1);

      // The k halvings, replayed on grid indices. `divisions` is a power of two, so
      // (indexLo + indexHi) / 2 is exact integer arithmetic throughout and index m is
      // rows[m - 1] (the grid holds the interior points only - index 0 is `lo` and index
      // `divisions` is `hi`, both already decided).
      let indexLo = 0;
      let indexHi = divisions;
      while (indexHi - indexLo > 1) {
        const mid = (indexLo + indexHi) / 2;
        if (Math.sign(rows[mid - 1].speed) === signLo) indexLo = mid; else indexHi = mid;
      }
      if (indexLo > 0) lo = rows[indexLo - 1].jd;
      if (indexHi < divisions) hi = rows[indexHi - 1].jd;
    }
    return (lo + hi) / 2;
  }
...
export function findContacts({ provider, segments, stations, natalLongitude, aspectAngle, orbAllowed, startJd, endJd }) {
  ...
  //
  // Which side each crossing came from is kept, not discarded after the merge: it fixes the
  // sign of g there (`-orbAllowed` on the lower target, `+orbAllowed` on the upper), which
  // is half of what the in-orb loop below needs to settle an interval without sampling.
  const orbCrossings = [
    ...enumerateCrossings(provider, segments, targetBase - orbAllowed).map((c) => ({ ...c, lower: true })),
    ...enumerateCrossings(provider, segments, targetBase + orbAllowed).map((c) => ({ ...c, lower: false })),
  ].sort((a, b) => a.jd - b.jd);

  // In-orb state timeline: the window cut at every orb-boundary crossing. Adjacent in-orb
  // intervals merge into one episode as the loop runs, since boundaries are contiguous and
  // sorted.
  //
  // Each interval's state used to be decided by reading |g| at its MIDPOINT - one swetest
  // spawn per interval, and a third of this whole search's spawns (SUP-391: 412 of 1,286 on
  // a 1-year transit aspect call, second only to crossing refinement itself). No interval
  // contains an orb-boundary crossing, by construction - they ARE the boundaries - so the
  // state is constant across each one and every number needed to name it is already in hand:
  //
  //   window start - |g| at startJd, read anyway for closest_approach's truncated-edge
  //     candidate, settles the first interval;
  //   parity - crossing an orb boundary flips in/out, so a later interval's state is the
  //     negation of the one before it;
  //   slope - the crossing's own `speed` IS dg/dt there, and its side fixes the sign of g,
  //     so |g| is shrinking (an entry) or growing (an exit) by arithmetic alone.
  //
  // Parity and slope are independent derivations of the same bit, and BOTH must agree for
  // the interval to be settled for free. That is deliberately not a cheaper version of
  // "trusting crossing parity", which the midpoint probe existed to avoid: a missed or
  // duplicated crossing breaks parity while leaving slope intact (and vice versa), so a
  // disagreement is exactly the anomaly the probe was there to catch - and on a
  // disagreement, the probe still runs and still decides. Same fallback when a crossing's
  // speed is zero (a station sitting exactly on the boundary, where slope says nothing) or
  // when the previous interval was dropped as degenerate, leaving parity no predecessor.
  // The self-verification is kept; only the unconditional spawn is gone.
  const boundaries = [startJd, ...orbCrossings.map((c) => c.jd), endJd];
  const episodes = [];
  let open = null;
  let previousInOrb = null;
  for (let i = 0; i < boundaries.length - 1; i++) {
    const lo = boundaries[i];
    const hi = boundaries[i + 1];
    if (hi - lo < JD_TOLERANCE) {
      previousInOrb = null; // degenerate interval (coincident crossings) - parity loses its thread
      continue;
    }

    let inOrb;
    if (i === 0) {
      inOrb = gStart <= orbAllowed;
    } else {
      const opener = orbCrossings[i - 1];
      // g = -orbAllowed on the lower boundary and +orbAllowed on the upper, so |g| shrinks
      // (an entry) when the two signs match and grows (an exit) when they don't.
      const bySlope = opener.speed === 0 ? null : opener.lower === (opener.speed > 0);
      inOrb = bySlope !== null && previousInOrb !== null && bySlope !== previousInOrb
        ? bySlope
        : Math.abs(wrap180(provider.positionAt((lo + hi) / 2).longitude - targetMod)) <= orbAllowed;
    }
    previousInOrb = inOrb;
...
// a second one alongside it. A wrapped provider's `samplesFrom` (SUP-390) primes itself for
// the same reason, so "the same (body, jd) is never sampled twice" holds for the batched
// read as well - a grid row and a later scalar read of that same instant collapse to one
// spawn rather than sneaking past the cache because they arrived by different doors.
export function memoizeProvider(provider) {
  const samples = new Map();
  return {
    ...provider,
    samplesFrom: typeof provider.samplesFrom === 'function'
      ? (startJd, stepDays, count) => {
        const rows = provider.samplesFrom(startJd, stepDays, count);
        for (const row of rows) {
          if (!samples.has(row.jd)) samples.set(row.jd, { longitude: row.longitude, speed: row.speed });
        }
        return rows;
      }
      : undefined,
...
// That is also why this composition deliberately has no `samplesFrom`: the batched read's
// only consumer is refineStationJd, which by the same argument never runs here. index.js's
// relativeMovingProvider, whose pair rate DOES station, composes one.
