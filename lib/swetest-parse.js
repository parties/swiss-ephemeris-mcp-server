...
export function parsePlanetLine(line) {
  ...
  // Illuminated fraction of the disc (-fPZSBDl-, appended after the decimal longitude so
  // every earlier positional index stays intact). The Ecl. Obl. pseudo-row prints literal
  // 'nan' here, but that row returns above and never reaches this branch; guard anyway so a
  // non-finite reading is dropped rather than leaking NaN into a real body.
  const illuminatedFractionRaw = parts.length >= 7 ? parseFloat(parts[6]) : undefined;
  const illuminatedFraction = Number.isFinite(illuminatedFractionRaw) ? illuminatedFractionRaw : undefined;
...
// Parse one row of the four-column decimal `-fJPls` CSV used by the stepped scan
// (spec §1.1): "2461041.50,Mars           , 282.6881579,  0.7663630" -> JD, body name,
// longitude, speed. Rows interleave by body within each timestep when multiple `-p`
// codes are requested in one call, but JD+name identify the row unambiguously, so the
// caller keys off both rather than assuming grouping. Never parses DMS.
export function parseStepRow(line) {
  ...
  if (!name || Number.isNaN(jd) || Number.isNaN(longitude) || Number.isNaN(speed)) return null;

  return { jd, name, longitude, speed };
}

// Parse the tab-delimited output of `swetest -lunecl`/`-solecl` (spec §1.5), which may
// contain several 3-line eclipse blocks back to back. Only line 1 of each block is
// parsed - the following two lines (contact times, geographic position) carry data this
// server doesn't surface and always start with a tab/space, which is exactly what marks
// them as continuation lines rather than a new block.
//
// Line 1 has 6 comma/tab fields for a lunar eclipse and 7 for solar (solar inserts a
// core-shadow-width field before the magnitudes) - the two are never forced into one
// shape. Type names are multi-token, e.g. "penumb. lunar eclipse", so lunar/solar is
// decided by substring match on the type text, not by counting tokens or trusting field
// count alone.
export function parseEclipseBlock(rawOutput) {
  const eclipses = [];
  ...
  for (const line of rawOutput.split('\n')) {
    ...
    const eclipseType = fields[0];
    const isSolar = /solar/i.test(eclipseType);
    const isLunar = /lunar/i.test(eclipseType);
    if (!isSolar && !isLunar) continue;
    if (isSolar && fields.length < 7) continue;
    if (isLunar && fields.length < 6) continue;

    const magnitudesField = isSolar ? fields[4] : fields[3];
    const sarosField = isSolar ? fields[5] : fields[4];
    const jdField = isSolar ? fields[6] : fields[5];

    const sarosMatch = sarosField.match(/saros\s+(\d+)\/(\d+)/);
    const jd = parseFloat(jdField);
    if (Number.isNaN(jd)) continue;

    eclipses.push({
      eclipse_type: eclipseType,
      jd,
      magnitudes: magnitudesField.split('/').map(Number),
      saros_series: sarosMatch ? parseInt(sarosMatch[1], 10) : null,
      saros_number: sarosMatch ? parseInt(sarosMatch[2], 10) : null,
    });
  }
