...
**Returns:**
- `planets`: Positions of all planets and celestial bodies. Each entry has `longitude`, `sign`, `degree`, `speed`, `ecliptic_latitude` (decimal degrees, + = north of the ecliptic), `declination` (decimal degrees, + = north of the celestial equator), `out_of_bounds` (`true` when `|declination|` exceeds the obliquity of the date), `out_of_bounds_by` (decimal degrees past the boundary when out of bounds, `null` otherwise), and `illuminated_fraction` (fraction of the disc illuminated, 0-1, read from swetest's phase column). All 17 bodies carry all five fields uniformly, including North Node (`ecliptic_latitude: 0`, always in bounds). The Sun is hard-coded `out_of_bounds: false`: its ~0.5″ apparent ecliptic latitude (light-time and aberration) can push its apparent declination a fraction of an arcsecond past true obliquity right at a solstice, which would otherwise flag the body that defines the boundary as having left it.
...
- `moon_phase` (omitted, not `null`, when Sun or Moon are unavailable — e.g. the missing-ephemeris path — same precedent as the ARMC declination omission above): `{ phase, elongation, illuminated_fraction, phase_scheme }`, keyed on the Moon − Sun ecliptic longitude difference under an eight-phase scheme (New, Crescent, First Quarter, Gibbous, Full, Disseminating, Last Quarter, Balsamic — each a 45° band). Bands **start at** the exact aspect (New = 0°–45°) rather than centering on it the way an almanac does (New = 337.5°–22.5°, which names a different phase near every boundary); `phase_scheme` echoes `"8-phase, bands start at exact aspect"` so a consumer never has to assume which convention produced the name. `elongation` (0–360, normalized) is derived from the parsed ecliptic longitudes, not swetest's `*` column — that column is the true 3D angular separation and can diverge by several degrees when the Moon has significant ecliptic latitude, enough to land in the wrong band. `illuminated_fraction` here is the Moon's own `illuminated_fraction`, repeated for convenience.
...
**Returns:**
- All fields from `calculate_planetary_positions` (`planets`, `houses`, `chart_points`, `additional_points`, `obliquity`, `obliquity_type`, `datetime`, `coordinates`, `house_system`, `moon_phase` when Sun and Moon are both available)
